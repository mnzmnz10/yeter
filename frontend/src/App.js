...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-25 to-teal-50">
      ...
      {/* Package Products Selection Dialog */}
      <Dialog open={showPackageProductsDialog} onOpenChange={setShowPackageProductsDialog}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Pakete Ürün Ekle: {selectedPackageForProducts?.name}</DialogTitle>
            <DialogDescription>
              Pakete eklemek istediğiniz ürünleri seçin ve adet belirleyin
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 max-h-96 overflow-y-auto">
              {products.map((product) => {
                ...
                return (
                  <div key={product.id} className={`border rounded-lg p-3 ${isSelected ? 'border-teal-300 bg-teal-50' : 'border-gray-200'}`}>
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        checked={isSelected}
          ...
          <DialogFooter>
            <div className="flex justify-between items-center w-full">
              <div className="text-sm text-slate-600">
                {packageSelectedProducts.size} ürün seçildi
              </div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setShowPackageProductsDialog(false)}>
                  İptal
                </Button>
                <Button 
                  onClick={() => addProductsToPackage(selectedPackageForProducts.id, packageSelectedProducts)}
                  className="bg-teal-600 hover:bg-teal-700"
